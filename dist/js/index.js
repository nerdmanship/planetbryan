...
var Flame = function () {
  ...
  _createClass(Flame, [{
    ...
    value: function animate() {
      ...
      var y = [195, spread(185, 1), spread(180, 1), spread(175, 1), spread(config.flames.height, 25)];

      var x = [spread(xMin, 18), spread(xMin, 8), spread(4, 2), spread(4, 2), spread(xMax + 100, 40)];

      var s = [0.5, spread(3.5, 1), spread(1, 0.3), spread(4, 1), spread(1, 0.5)];

      var c = ["hsl(" + random(5, 15) + ", " + random(75, 85) + "%, 60%)", "hsl(45, 100%, " + random(75, 95) + "%)", // gray
      "hsl(45,0%,40%)" // gray
      ];
      ...
      // SCALE
      .to(f, timeS[0], { scale: s[1], ease: Power1.easeOut }, "start").to(f, timeS[1], { scale: s[2], ease: Power1.easeInOut }, timeS[0]).to(f, timeS[2], { scale: s[3], ease: Power1.easeInOut }, timeS[0] + timeS[1]).to(f, totTime - (timeS[0] + timeS[1] + timeS[2]), { scale: s[4], ease: Power1.easeOut }, timeS[0] + timeS[1] + timeS[2])

      // COLOR
      .to(f, timeC[0], { fill: c[1], ease: Power1.easeInOut }, "start").to(f, timeC[1], { fill: c[2], ease: Power1.easeInOut }, timeS[0] + timeS[1])

      // ALPHA
      .to(f, timeC[1], { autoAlpha: 0.2, ease: Power1.easeOut }, timeS[0] + timeS[1]).to(f, totTime - (timeS[0] + timeS[1] + timeC[1]), { autoAlpha: 0, ease: Power2.easeOut }, timeS[0] + timeS[1] + timeC[1]);
    }
  }]);
